...
export default function ProjectDocumentsPage({ project: propProject }) {
    const { id } = useParams(); 
    // Надійніше отримання ID (спочатку пропс, потім URL, потім 0)
    ...
    const fetchDocuments = useCallback(async () => {
        ...
        try {
            ...
            if (error.name !== 'AbortError') {
                ...
                // Не показуємо тост помилки при першому завантаженні, щоб не лякати, якщо пусто
            }
        } finally {
            setLoading(false);
        }
    ...
    const handleUpload = async (e) => {
        ...
        if (!projectId) {
            showToast('error', "Помилка: ID проекту не знайдено");
            return;
        }
        ...
        formData.append('object_number', String(projectId)); // Примусово String
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                showToast('error', result.message || "Помилка завантаження");
            }
        } catch (error) {
            console.error("Upload failed:", error);
            showToast('error', "Помилка з'єднання або сервера");
        } finally {
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 sticky top-6">
                        ...
                        <form onSubmit={handleUpload} className="space-y-4">
                            <div className="relative">
                                ...
                                <div className="relative">
                                    <select 
                                        ...
                                        className={`w-full p-2.5 pr-8 rounded-xl border text-sm focus:ring-2 outline-none transition font-medium text-slate-700 appearance-none cursor-pointer ${typeError ? 'border-red-500 bg-red-50 focus:ring-red-200' : 'border-slate-200 bg-slate-50 focus:ring-indigo-500 focus:bg-white'}`}
                                    >
                                ...
                                {typeError && <p className="text-[10px] text-red-500 mt-1 font-bold animate-pulse">Необхідно обрати тип документа!</p>}
                            </div>
                            ...
                            <div 
                                className={`relative border-2 border-dashed rounded-xl p-6 transition-all text-center cursor-pointer group ${isDragging ? 'border-indigo-500 bg-indigo-50 scale-[1.02]' : 'border-slate-300 hover:border-indigo-400 hover:bg-slate-50'}`}
                    ...
                    <div className="flex justify-between items-center bg-white p-3 rounded-2xl shadow-sm border border-slate-200">
                        ...
                        <div className="relative min-w-[200px] md:min-w-[250px]">
                            ...
                            >
                                ...
                                {DOC_TYPES.map(type => (
                                    <option key={type} value={type}>{type} ({counts[type] || 0})</option>
                                ))}
                            </select>
                            <FaChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none text-xs"/>
                        </div>
                                ...
                                <AnimatePresence mode="popLayout">
                                    {filteredDocuments.map((doc) => (
                                        <DocumentCard 
                                            key={doc.id} 
                                            doc={doc} 
                                            onPreview={() => setPreviewUrl(driveViewUrl(doc.id))} 
                                        />
                                    ))}
                                </AnimatePresence>
                            </div>
            ...
            <AnimatePresence>
                {toast && (
                    <motion.div 
                        ...
                        className={`fixed bottom-10 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-3 font-bold text-sm z-[90] ${toast.type === 'error' ? 'bg-red-500 text-white' : 'bg-slate-900 text-white'}`}
                    >
...
const DocumentCard = ({ doc, onPreview }) => {
    ...
    // Використовуємо наш хелпер для прев'ю
    const thumbnail = isImage ? driveThumbUrl(doc.id) : null;
            ...
            <div className="flex-1 bg-slate-50 relative overflow-hidden flex items-center justify-center p-4 cursor-pointer" onClick={onPreview}>
                {isImage ? (
                    <img 
                        ...
                        loading="lazy"
                        onError={(e) => { e.currentTarget.style.display='none'; e.currentTarget.parentElement.innerHTML += '<div class="text-xs text-slate-400">Помилка фото</div>' }}
                    />
                ) : (
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3 backdrop-blur-[2px]">
                    <button className="p-3 bg-white text-slate-900 rounded-full shadow-lg hover:scale-110 transition" title="Перегляд" onClick={(e) => { e.stopPropagation(); onPreview(); }}>
                        <FaEye/>
                    </button>
                    {doc.webContentLink && (
                        <a href={doc.webContentLink} onClick={(e) => e.stopPropagation()} className="p-3 bg-white text-emerald-600 rounded-full shadow-lg hover:scale-110 transition" title="Завантажити">
                            <FaDownload/>
                        </a>
                    )}
                </div>

...
import {
  ...
  FaChevronLeft, FaChevronRight, FaCheck, FaInfoCircle, FaHardHat,
  FaUserFriends, FaUserTie,
  FaUserEdit,
  // Нові іконки для відповідності стандарту
  FaMapMarkerAlt, FaDraftingCompass, FaFileInvoiceDollar, FaTruckLoading, FaSolarPanel, FaBroadcastTower, FaFileSignature,
  FaBoxOpen,
  FaArrowRight,
} from "react-icons/fa";
...
// ОНОВЛЕНИЙ СПИСОК ЕТАПІВ (Синхронізовано з inst2.jsx та PWT.jsx)
...
// Словник для мапінгу старих ключів на нові
const STAGE_ALIASES = {
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className="fixed top-20 right-4 z-[100] max-w-[90vw]"
        >
...
export default function InstallationsPage() {
  ...
  // ✅ беремо роль/employeeId з AuthProvider
  const { employee, user } = useAuth();
  const role = employee?.role || null;
  const myEmployeeId = employee?.custom_id || null;
  ...
  // ✅ ОНОВЛЕНИЙ HELP: враховує аліаси
  ...
  // --- LOAD DICTIONARIES (через API) ---
  useEffect(() => {
    const loadStaticData = async () => {
      try {
        const meta = await apiGet("/api/installations/meta");
  ...
  const loadProjects = useCallback(async () => {
    ...
    try {
      const qs = new URLSearchParams({
        ...
        onlyMine: onlyMyProjects && myEmployeeId ? "1" : "0",
      });

      const resp = await apiGet(`/api/installations?${qs.toString()}`);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [debouncedSearchTerm, statusFilter, paymentStatusFilter, companyFilter, onlyMyProjects]);

  // --- Handlers ---
  const handleDetailsClick = (project) => navigate(`/project/${project.custom_id}`);

  // ✅ головне: відкриття модалки
  ...
  // ✅ щоб на мобільному не “з’їдало” ID — ставимо ID на початку
  const handleClientSelect = (client) => {
    setFormData((prev) => ({ ...prev, client_id: client.custom_id }));
    const nm = client.company_name || client.name || "";
    setClientSearch(`ID: ${client.custom_id} — ${nm}`);
  };

  const handleEmployeeSelect = (emp) => {
    setFormData((prev) => ({ ...prev, responsible_emp_id: emp.custom_id }));
    setEmployeeSearch(`ID: ${emp.custom_id} — ${emp.name}`);
  };

  // Покращений пошук: name/company_name + custom_id
  ...
  const handleSubmit = async () => {
    if (!formData.client_id) {
      setFormErrors({ client_id: "Оберіть клієнта" });
      return;
    }
    ...
    try {
      ...
      // ✅ create/update через API
      if (editingProject) {
        await apiPatch(`/api/installations/${editingProject.custom_id}`, sanitized);
      } else {
        await apiPost("/api/installations", sanitized);
      }
    ...
    return (
      <div className="flex items-center justify-center space-x-4 py-4">
        <button
          ...
          className="p-2 disabled:opacity-50 bg-white rounded-md shadow-sm border border-gray-200"
        >
          <FaChevronLeft />
        </button>
    ...
    <div className="p-4 sm:p-8 space-y-6 max-w-[1600px] mx-auto pb-safe min-h-[calc(100vh-80px)] flex flex-col text-slate-800">
      ...
      <div className="flex flex-col gap-4 flex-none">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
          ...
          {/* ✅ НЕ блокуємо кнопку — щоб модалка завжди відкривалась */}
          <button
            onClick={handleAddProject}
            className="flex items-center justify-center gap-2 px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-bold shadow-lg hover:bg-indigo-700 active:scale-95 transition-all w-full sm:w-auto"
          >
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between gap-3">
            {myEmployeeId ? (
              <div className="bg-white p-1 rounded-xl border border-slate-200 flex items-center shadow-sm flex-1 sm:flex-none">
                <button
                  ...
                  className={`flex-1 sm:flex-none px-4 py-2.5 rounded-lg text-sm font-bold transition-all ${
                    !onlyMyProjects ? "bg-indigo-100 text-indigo-700 shadow-sm" : "text-slate-500 hover:text-slate-700"
                  }`}
                >
                  Всі
                </button>
                <button
                  ...
                  className={`flex-1 sm:flex-none px-4 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${
                    onlyMyProjects ? "bg-indigo-600 text-white shadow-md" : "text-slate-500 hover:text-slate-700"
                  }`}
                >
            ...
            <button
              ...
              className={`flex-shrink-0 flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-semibold transition shadow-sm border ${
                showFilters ? "bg-slate-800 text-white border-slate-800" : "bg-white text-slate-700 border-slate-200 hover:bg-slate-50"
              }`}
            >
                    ...
                    {[{ v: "active", l: "Активні" }, { v: "all", l: "Всі" }, { v: "planning", l: "План" }, { v: "in_progress", l: "Робота" }, { v: "on_hold", l: "Пауза" }, { v: "completed", l: "Фініш" }, { v: "cancelled", l: "Відміна" }].map((s) => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${
                          statusFilter === s.v ? "bg-indigo-100 text-indigo-700" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                        }`}
                      >
                        {s.l}
                      </button>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[{ v: "all", l: "Всі" }, { v: "pending", l: "Очікує" }, { v: "partial", l: "Частково" }, { v: "paid", l: "Сплачено" }, { v: "overdue", l: "Борг" }].map((s) => (
                      <button
                        key={s.v}
                        onClick={() => setPaymentStatusFilter(s.v)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${
                          paymentStatusFilter === s.v ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                        }`}
                      >
                        {s.l}
                      </button>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[{ v: "all", l: "Всі" }, ...ALLOWED_COMPANIES.map((c) => ({ v: c, l: c }))].map((c) => (
                      <button
                        key={c.v}
                        onClick={() => setCompanyFilter(c.v)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${
                          companyFilter === c.v ? "bg-purple-100 text-purple-700" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                        }`}
                      >
                        {c.l}
                      </button>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="h-64 bg-white rounded-xl animate-pulse shadow-sm border border-slate-100" />
            ))}
          </div>
        ) : projects.length === 0 ? (
            ...
            {projects.map((project) => {
              ...
              return (
                ...
                >
                  <div>
                    <div className="flex justify-between items-start gap-3 mb-4">
                      <div className="pr-1 flex-1 min-w-0">
                        <h3 className="text-lg font-bold text-slate-800 leading-tight line-clamp-2" title={project.name}>
                          {project.name || project.client?.company_name || "Без назви"}
                        </h3>
                        <span className="text-xs font-mono font-medium text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded mt-1 inline-block">
                          #{project.custom_id}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-col items-end gap-2 min-w-[140px] flex-shrink-0">
                        <div
                          ...
                          className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold text-white shadow-sm ${statusInfo.color}`}
                        >
                    ...
                    <div className="space-y-3 mb-6">
                      ...
                      <div className="flex items-center gap-3 text-sm text-slate-600">
                        <div className="w-5 flex justify-center text-indigo-500">
                          <FaHardHat size={16} />
                        </div>
                        <span>
                          Відповідальний:{" "}
                  ...
                  {/* ✅ НИЖНІЙ РЯДОК: тільки оплата + детальна */}
                  <div className="border-t border-slate-100 pt-4 flex justify-between items-center gap-3">
                    <div className={`text-xs font-bold px-3 py-1.5 rounded-lg border ${paymentInfo.color}`}>
                      {paymentInfo.label}
                    </div>

                    <button
                      onClick={() => handleDetailsClick(project)}
                      className="ml-auto inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold transition-colors"
                      title="Детальна інформація"
                    >
                      Детальна інформація <FaArrowRight className="text-[11px]" />
                    </button>
                  </div>
                </motion.div>
              );
      ...
      {/* ✅ FORM MODAL (твій, повернув повністю) */}
      <AnimatePresence>
        {showProjectForm && (
          <motion.div
          ...
          >
            <motion.div
              className="bg-white rounded-2xl p-4 sm:p-8 w-full max-w-lg md:max-w-4xl shadow-2xl border border-gray-200 my-8 max-h-[90vh] flex flex-col"
            ...
            >
              <div className="mb-6">
                <h2 className="text-2xl sm:text-3xl font-bold text-slate-800">
                  {editingProject ? "Редагувати" : "Новий проект"}
                </h2>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 overflow-y-auto pr-2 flex-grow">
                ...
                <div className="md:col-span-2 bg-slate-50 p-4 rounded-xl border border-slate-200 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {/* ОНОВЛЕНО: використовуємо WORKFLOW_STAGES */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Етап</label>
                    <select
                      value={formData.workflow_stage || "tech_review"}
                      onChange={(e) => handleInputChange("workflow_stage", e.target.value)}
                      className="w-full border border-gray-300 rounded-xl px-4 py-3 bg-white text-indigo-900 font-medium"
                    >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => handleInputChange("working_company", e.target.value)}
                    className="w-full border border-gray-300 rounded-xl px-4 py-3 bg-white"
                  >
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">GPS</label>
                  <input
                    type="url"
                    value={formData.gps_link || ""}
                    onChange={(e) => handleInputChange("gps_link", e.target.value)}
                    className="w-full border border-gray-300 rounded-xl px-4 py-3"
                  />
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Координати</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Потужність (кВт)</label>
                  <input
                    type="number"
                    step="0.1"
                    min="0"
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (formData.responsible_emp_id) handleInputChange("responsible_emp_id", "");
                    }}
                    placeholder="Пошук..."
                    className="w-full border border-gray-300 rounded-xl px-4 py-3"
                  />
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Старт</label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Фініш</label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Вартість (USD)</label>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Сплачено (USD)</label>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-gray-200 mt-6 flex-shrink-0">
                ...
                <button
                  ...
                  disabled={submitting}
                  className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-medium transition hover:bg-indigo-700 flex items-center space-x-2"
                >

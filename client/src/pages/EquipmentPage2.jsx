...
// Переконайся, що шлях правильний (залежно від структури папок)
...
export default function EquipmentPage() {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const data = await apiRequest("/api/equipment2/data");
    ...
    const deleteType = (id) => {
        setConfirmDialog({
            ...
            onConfirm: () => { setConfirmDialog({ isOpen: false }); handleCrud(apiCrud(`/api/equipment2/equipment/${id}`, { method: "DELETE" }), 'Видалено'); }
        });
    };
    ...
    const removeEquipment = (id) => {
        setConfirmDialog({
            ...
            onConfirm: () => { setConfirmDialog({ isOpen: false }); handleCrud(apiCrud(`/api/equipment2/installed-equipment/${id}`, { method: "DELETE" }), 'Знято'); }
        });
    };
                ...
                <AnimatePresence>
                    ...
                    {modal.type === 'assign' && (
                        <AssignEquipmentModal 
                            ...
                            // Перехід до сканера прямо з модалки (для зручності)
        ...
        <Modal closeModal={closeModal} size="md">
            <form onSubmit={handleSubmit} className="flex flex-col h-full overflow-hidden">
                <div className="flex-none p-6 border-b border-slate-100 flex justify-between items-start">
                    ...
                    {/* Посилання на сканер для зручності */}
                    <button 
                        type="button" 

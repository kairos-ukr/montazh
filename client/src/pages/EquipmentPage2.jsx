...
// ✅ Підключаємо твій HTTP-клієнт (URL вже всередині http.js)
...
const InputField = memo(({ icon, label, error, showPassword, onTogglePassword, ...props }) => (
  <div>
    ...
    <div className="relative">
      {icon && (
        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none text-lg">
          {icon}
        </span>
      )}
      <input
        ...
        className={`w-full border rounded-xl ${
          icon ? "pl-12" : "pl-4"
        } pr-4 py-3.5 text-base sm:text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition bg-white border-slate-200 ${
...
const SelectField = memo(({ icon, label, children, error, ...props }) => (
  <div>
    ...
    <div className="relative">
      ...
      <select
        {...props}
        className="w-full appearance-none border rounded-xl pl-12 pr-10 py-3.5 text-base sm:text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white border-slate-200"
      >
        {children}
      </select>
...
const SearchableSelect = memo(
  ({ options, value, onChange, icon, label, placeholder = "Оберіть...", disabled = false }) => {
      ...
      <div ref={wrapperRef} className={disabled ? "opacity-50 pointer-events-none" : ""}>
        ...
        <div className="relative">
          <button
            ...
            className="w-full border border-slate-200 rounded-xl pl-12 pr-10 py-3.5 text-base sm:text-sm text-left bg-white flex items-center justify-between focus:ring-2 focus:ring-indigo-500 outline-none"
          >
            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-lg">
              {icon}
            </span>
            <span className={`truncate ${selectedOption ? "text-slate-800" : "text-slate-500"}`}>
              {selectedOption ? selectedOption.label : placeholder}
            </span>
            <Fa.FaChevronDown
              className={`transition-transform ${isOpen ? "rotate-180" : ""} text-slate-400 flex-shrink-0`}
            />
          </button>
          ...
          <AnimatePresence>
            {isOpen && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10 }}
                className="absolute z-20 top-full mt-2 w-full bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden max-h-60 overflow-y-auto"
              >
                <div className="p-2 sticky top-0 bg-white border-b border-slate-100">
                  <input
                    ...
                    className="w-full px-3 py-2 border border-slate-200 rounded-lg text-base sm:text-sm outline-none focus:border-indigo-500"
                  />
                </div>
                <ul>
                  {filteredOptions.length > 0 ? (
                    filteredOptions.map((opt) => (
                      <li
                        ...
                        className="px-4 py-3 hover:bg-indigo-50 cursor-pointer text-sm text-slate-700 border-b border-slate-50 last:border-0"
                      >
                        {opt.label}
                      </li>
                    ))
    ...
    <div>
      ...
      <div className="border border-slate-200 rounded-xl bg-white p-2 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-transparent transition-all">
        <div className="flex flex-wrap gap-2 mb-2">
          {serials.map((sn, idx) => (
            <span
              key={idx}
              className="bg-indigo-50 text-indigo-700 px-2 py-1 rounded-lg text-xs font-bold flex items-center gap-1 border border-indigo-100"
            >
              {sn}
              <button
                type="button"
      ...
      {serials.length < max && (
        <p className="text-[10px] text-amber-500 mt-1 ml-1 font-medium">
          Ще можна додати {max - serials.length} шт. (не обов&apos;язково)
        </p>
      )}
    </div>
  );
      ...
      {notification.message && (
        <motion.div
          ...
          className={`
            ...
            sm:left-auto sm:right-6 sm:w-auto sm:min-w-[300px]
        ...
        >
          ...
          <button onClick={onDismiss} className="opacity-80 hover:opacity-100 p-1">
            <Fa.FaTimes />
          </button>
        </motion.div>
      )}
    ...
    <Modal closeModal={onClose} size="sm">
      <div className="p-6 text-center">
        ...
        <div className="flex justify-center gap-3">
          ...
          <button
            onClick={onConfirm}
            className="px-5 py-2.5 bg-red-500 text-white rounded-xl font-bold hover:bg-red-600"
          >
            Видалити
          </button>
        </div>
...
// ✅ Обгортки під твій apiGet/apiPost/apiPatch/apiDelete, щоб НЕ міняти логіку handleCrud
async function apiCrud(path, { method = "POST", body } = {}) {
  try {
    const m = (method || "GET").toUpperCase();
    let data = null;

    if (m === "GET") data = await apiGet(path);
    else if (m === "POST") data = await apiPost(path, body ?? {});
    else if (m === "PATCH") data = await apiPatch(path, body ?? {});
    else if (m === "DELETE") {
      data = await apiDelete(path);
    } else {
    ...
    return { data: null, error: { message: e?.message || "Request error" } };
  }
}
...
export default function EquipmentPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      // ✅ Було fetch("/api/equipment2/data"), стало через http.js
      const data = await apiGet("/api/equipment2/data");
      setEquipment(data?.equipment || []);
      setInstallations(data?.installations || []);
      setInstalledEquipment(data?.installedEquipment || []);
  ...
  const filteredInstallations = useMemo(() => {
    ...
    return installationsWithEquipment.filter(
      (inst) =>
  ...
  const handleTypeSubmit = (data) => {
    ...
    const req = modal.data?.id
      ? apiCrud(`/api/equipment2/equipment/${modal.data.id}`, { method: "PATCH", body: payload })
      : apiCrud(`/api/equipment2/equipment`, { method: "POST", body: payload });
  ...
  const deleteType = (id) => {
    setConfirmDialog({
      isOpen: true,
      title: "Видалити тип?",
      message: "Це вплине на всі записи.",
      onConfirm: () => {
        setConfirmDialog({ isOpen: false });
        handleCrud(apiCrud(`/api/equipment2/equipment/${id}`, { method: "DELETE" }), "Видалено");
      },
    });
  };
  ...
  const handleAssign = (data) => {
    ...
    handleCrud(apiCrud(`/api/equipment2/installed-equipment`, { method: "POST", body: payload }), "Додано на об'єкт");
  };
  ...
  const handleUpdateDetails = (data) => {
    ...
    handleCrud(apiCrud(`/api/equipment2/installed-equipment/${id}`, { method: "PATCH", body: payload }), "Оновлено");
  };

  const removeEquipment = (id) => {
    setConfirmDialog({
      isOpen: true,
      title: "Зняти обладнання?",
      message: "Ви впевнені?",
      onConfirm: () => {
        setConfirmDialog({ isOpen: false });
        handleCrud(apiCrud(`/api/equipment2/installed-equipment/${id}`, { method: "DELETE" }), "Знято");
      },
    });
  };
        ...
        {activeTab === "installations" && (
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-base sm:text-sm shadow-sm"
            />
          </div>
        )}
        ...
        {activeTab === "catalog" && (
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex bg-white border border-slate-200 p-1 rounded-xl overflow-x-auto no-scrollbar flex-1">
              {[
                { id: "all", l: "Всі" },
                { id: "inverter", l: "Інвертори" },
                { id: "battery", l: "АКБ" },
                { id: "solar_panel", l: "Панелі" },
                { id: "logger", l: "Логери" },
              ].map((cat) => (
                <button
                  key={cat.id}
            ...
            <button
              onClick={() => setModal({ type: "addType", data: null })}
              className="w-full sm:w-auto self-end flex items-center justify-center gap-2 py-2.5 px-5 bg-indigo-600 text-white rounded-xl font-bold shadow-lg hover:bg-indigo-700 active:scale-95 transition-all text-sm"
            >
      ...
      <div className="flex-1">
        <AnimatePresence mode="wait">
          {activeTab === "installations" ? (
            ...
            >
              {loading ? (
                <div className="col-span-full text-center py-20 text-slate-400">Завантаження...</div>
                ...
                paginatedInstallations.map((inst) => (
                  ...
                  >
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <button
                                  onClick={() => setModal({ type: "viewDetails", data: item })}
                                  className="p-1.5 text-blue-500 hover:bg-blue-50 rounded-lg"
                                >
                                  <Fa.FaEye />
                                </button>
                                <button
                                  onClick={() => setModal({ type: "editDetails", data: item })}
                    ...
                    {/* ОНОВЛЕНИЙ ФУТЕР КАРТКИ */}
                    <div className="flex gap-2">
                      <button
                        onClick={() => setModal({ type: "assign", data: inst })}
                        className="flex-1 py-2.5 border-2 border-dashed border-indigo-200 text-indigo-600 rounded-xl font-bold hover:bg-indigo-50 transition-colors flex items-center justify-center gap-2"
                      >
                        <Fa.FaPlus /> Додати
                      </button>
                      <button
                        onClick={() => setModal({ type: "scan", data: inst })}
                        className="px-4 py-2.5 bg-indigo-50 text-indigo-600 rounded-xl border border-indigo-100 hover:bg-indigo-100 transition-colors"
                        title="Сканувати фото"
                      >
                        <Fa.FaCamera size={18} />
                      </button>
                    </div>
                        ...
                        return (
                          <tr key={type.id} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="p-4 text-right">
                              <button
                                onClick={() => setModal({ type: "editType", data: type })}
      ...
      <AnimatePresence>
        ...
        {modal.type === "assign" && (
          <AssignEquipmentModal
            ...
            switchToScan={() => setModal({ type: "scan", data: modal.data })}
          />
        )}

        {/* НОВА МОДАЛКА СКАНУВАННЯ */}
        {modal.type === "scan" && (
          <OCRScanModal closeModal={() => setModal({ type: null })} installation={modal.data} showNotification={showNotification} />
        )}

        {(modal.type === "addType" || modal.type === "editType") && (
          <TypeFormModal closeModal={() => setModal({ type: null })} onSubmit={handleTypeSubmit} initialData={modal.data} loading={loading} />
        )}
...
function AssignEquipmentModal({ closeModal, installation, onAssign, equipmentCatalog, showNotification, switchToScan }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.equipment_id) return showNotification("Оберіть тип", "error");
    onAssign(formData);
  };
    ...
    <Modal closeModal={closeModal} size="md">
      <form onSubmit={handleSubmit} className="flex flex-col h-full overflow-hidden">
        <div className="flex-none p-6 border-b border-slate-100 flex justify-between items-start">
          <div>
            <h3 className="text-xl font-bold text-slate-800">Додати на об&apos;єкт</h3>
            <p className="text-sm text-indigo-600">{installation.name}</p>
          </div>

          <button
            type="button"
            onClick={switchToScan}
            className="text-xs font-bold text-indigo-600 bg-indigo-50 px-3 py-2 rounded-lg hover:bg-indigo-100 flex items-center gap-2"
          >
            <Fa.FaCamera /> Фотосканер
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold ${
                  !categoryFilter ? "bg-slate-800 text-white" : "bg-slate-100 text-slate-600"
                }`}
              >
                Всі
              </button>
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold ${
                    categoryFilter === cat.id ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-600"
                  }`}
                >
                  {cat.l}
                </button>
          ...
          <SearchableSelect
            ...
            placeholder={categoryFilter ? "Оберіть зі списку..." : "Спочатку оберіть категорію або шукайте..."}
          />
        ...
        <div className="flex-none p-6 flex justify-end gap-3 bg-slate-50 border-t border-slate-100 pb-safe">
          <button
            type="button"
            onClick={closeModal}
            className="px-4 py-2 bg-white border border-slate-200 rounded-lg font-bold text-slate-600"
          >
            Скасувати
          </button>
          <button type="submit" className="px-4 py-2 bg-indigo-600 text-white rounded-lg font-bold shadow-lg">
            Додати
          </button>
        </div>
      </form>
    ...
    <Modal closeModal={closeModal} size="md">
      <form onSubmit={(e) => { e.preventDefault(); onSubmit(formData); }} className="flex flex-col h-full overflow-hidden">
        <div className="flex-none p-6 border-b border-slate-100">
          <h3 className="text-xl font-bold text-slate-800">{initialData ? "Редагувати тип" : "Новий тип"}</h3>
        </div>
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          <InputField
            label="Назва"
          ...
          >
            ...
            <option value="solar_panel">Панель</option>
        ...
        <div className="flex-none p-6 flex justify-end gap-3 bg-slate-50 border-t border-slate-100 pb-safe">
          <button type="button" onClick={closeModal} className="px-4 py-2 bg-white border border-slate-200 rounded-lg font-bold text-slate-600">
            Скасувати
          </button>
          <button type="submit" disabled={loading} className="px-4 py-2 bg-indigo-600 text-white rounded-lg font-bold shadow-lg">
            Зберегти
          </button>
        </div>
      </form>
  ...
  return (
    <Modal closeModal={closeModal} size="sm">
      <div className="flex flex-col h-full overflow-hidden">
        <div className="flex-none p-6 border-b border-slate-100">
          <h3 className="text-lg font-bold text-slate-800">{item.equipment?.name}</h3>
        </div>
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            <div className="flex flex-col border-b pb-4 gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                {serialDisplay.split(", ").map((sn, i) => (
                  <span key={i} className="bg-slate-100 text-slate-700 px-2 py-1 rounded text-xs font-mono border border-slate-200">
                    {sn}
                  </span>
                ))}
              </div>
          ...
          {item.equipment?.category !== "solar_panel" && item.equipment?.category !== "battery" && (item.login || item.password) && (
            <div className="bg-slate-50 p-3 rounded-lg space-y-2 border border-slate-100">
              <div className="flex justify-between text-sm">
                <span className="text-slate-500">Логін</span>
                <span className="font-bold">{item.login}</span>
              </div>
              <div className="flex justify-between text-sm items-center">
                <span className="text-slate-500">Пароль</span>
                <div className="flex gap-2 items-center">
                  <span className="font-mono">{showPass ? item.password : "••••••"}</span>
        ...
        <div className="flex-none p-4 bg-slate-50 text-right pb-safe">
          <button onClick={closeModal} className="px-4 py-2 bg-white border border-slate-200 rounded-lg font-bold text-slate-600">
            Закрити
          </button>
        </div>
      </div>
    ...
    <Modal closeModal={closeModal} size="lg">
      <form onSubmit={handleSubmit} className="flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6">
          {!isPanel && <SerialNumberInput serials={serials} onChange={setSerials} max={qty} />}
          ...
          <SelectField
            ...
            icon={<Fa.FaUserTie />}
            value={formData.employee_custom_id}
            onChange={handleChange}
          >
            <option value="">Не вказано</option>
            {employees.map((e) => (
              <option key={e.custom_id} value={e.custom_id}>
                {e.name}
              </option>
            ))}
          </SelectField>
        ...
        <div className="flex-none flex justify-end gap-3 p-6 bg-slate-50/50 rounded-b-2xl border-t border-slate-100 pb-safe">
          <button type="button" onClick={closeModal} className="px-5 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold hover:bg-slate-50 transition">
            Скасувати
          </button>
          <button
            type="submit"

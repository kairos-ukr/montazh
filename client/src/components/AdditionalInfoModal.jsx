...
import { apiPost } from "../api/http"; // Імпорт твого хелпера
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                // Якщо currentUser.name нема, бекенд підставить email з токена або "Менеджер"
                author_name: currentUser?.name || currentUser?.email 
            };

            // Використовуємо твій хелпер apiPost
            // Він сам додає credentials: 'include', тому куки полетять на сервер
            await apiPost('/api/additional-info', payload);

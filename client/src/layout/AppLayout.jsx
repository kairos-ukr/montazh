...
// ✅ ВАЖЛИВО: підлаштуй шлях під свій проект
// приклади: "../context/AuthProvider" або "../auth/AuthProvider"
import { useAuth } from "../context/AuthProvider";

// ✅ ВАЖЛИВО: підлаштуй шлях до лого під свій проект
import logo from "../assets/logoCore1.png";
...
export default function AppLayout() {
  ...
  // Очікується, що твій AuthProvider дає ці поля:
  // user (або null), isLoading (bool), signOut()
  const { user, isLoading, signOut } = useAuth();

  // ✅ Захист приватних сторінок:
  // якщо не залогінений — перекидаємо на /auth
  useEffect(() => {
  ...
  const handleLogout = async () => {
    try {
      await signOut(); // має робити POST /api/auth/sign-out
    } finally {
      setIsOpen(false);
      navigate("/auth", { replace: true });
    }
  ...
  const menuItems = [
    ...
    { path: "/calendar", label: "Розклад", icon: FaCalendar },
    { path: "/weakends", label: "Вихідні", icon: FaUsers },
    { path: "/tasks", label: "Мікрозадачі", icon: FaTasks },
    { path: "/equipment", label: "Обладнання", icon: FaCog },
  ];

  // Поки auth стан вантажиться — щоб не було “миготіння”
  if (isLoading) {
    return (
  ...
  // Якщо це /auth — лейаут не показуємо взагалі (там своя сторінка)
    ...
    <div className="flex h-[100dvh] w-full bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative h-full">
        <header className="lg:hidden flex items-center justify-between px-5 py-3 bg-white border-b border-slate-200 flex-shrink-0 z-30 min-h-[64px]">
          <img src={logo} alt="K-Core" className="h-8 w-auto object-contain" />
          <button
